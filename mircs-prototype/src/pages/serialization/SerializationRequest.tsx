// REQ-0042, REQ-0044, REQ-0046, REQ-0047, REQ-0049, REQ-0051, REQ-0052, REQ-0113, REQ-0165, REQ-0220
...
export default function SerializationRequest() {
  ...
  function generateSerial(): string {
    ...
    return prefix + String(Math.floor(Math.random() * 900000000 + 100000000));
  }
    ...
    return (
      <div>
        ...
        <div className="card">
          ...
          <div className="alert alert-warning"><strong>⚠️ Timing Notice:</strong> Per 501 CMR regulations, serial numbers must be applied to the firearm within the required timeframe from manufacture or assembly.</div>
          <div className="alert alert-danger"><strong>⛔ Non-Transferable:</strong> The serial number assigned by DCJIS is non-transferable. Transfer to any other firearm is expressly prohibited.</div>
          <div style={{ textAlign: 'center', padding: '1.5rem 0' }}>
            ...
            <div style={{ fontSize: '0.8rem', color: '#616161', marginTop: '0.75rem' }}>Issued: {new Date().toLocaleDateString()} &nbsp;|&nbsp; Type: {firearmType} &nbsp;|&nbsp; {firstName} {lastName}</div>
          </div>
          ...
          <div className="alert alert-info" style={{ marginTop: '1rem' }}>
            Retain this document for your records. DCJIS will not mail a physical copy.
            <br /><br />
          ...
          <div style={{ marginTop: '1rem', padding: '1rem', background: '#f9f9f9', border: '1px solid #ddd', borderRadius: 4 }}>
            ...
            <p style={{ fontSize: '0.875rem', color: '#616161', marginBottom: '0.75rem' }}>If you no longer need this serial number, you may mark it as unused. Once marked unused, it cannot be reactivated or reused.</p>
  ...
  return (
    <div>
      ...
      <div className="alert alert-info">
        <strong>ℹ️ Serial Number Request System:</strong> Per M.G.L. c.140 §121C, DCJIS provides unique serial numbers for privately made firearms and firearms without valid serial numbers.
      </div>
      <div className="steps">
        ...
        <div className="card">
          ...
          <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
            {[
              ...
              <button key={val} onClick={() => setRequesterType(val)} style={{ flex: '1 1 220px', background: requesterType === val ? '#e8f0f9' : '#fff', border: `2px solid ${requesterType === val ? '#14558f' : '#d0d0d0'}`, borderRadius: 8, padding: '1rem', cursor: 'pointer', textAlign: 'left' }}>
            ...
            <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'flex-end', marginBottom: '1rem' }}>
              ...
              <div className="form-group" style={{ margin: 0 }}>
                ...
                <select className="form-control" value={licenseType} onChange={e => setLicenseType(e.target.value)} style={{ width: 280 }}>
                  <option value="">-- Select --</option>{LICENSE_TYPES.map(t => <option key={t} value={t}>{t}</option>)}
                </select>
              </div>
      ...
      {step === 3 && (
        <div className="card">
          ...
          <div className="form-row-3">
            <div className="form-group">
              ...
              <select className="form-control" value={firearmType} onChange={e => setFirearmType(e.target.value)}>
                <option value="">-- Select --</option>{FIREARM_TYPES.map(t => <option key={t} value={t}>{t}</option>)}
              </select>
            ...
            <div className="form-group">
              ...
              <select className="form-control" value={firearmMake} onChange={e => setFirearmMake(e.target.value)}>
                <option value="">-- Select --</option>{FIREARM_MAKES.map(m => <option key={m} value={m}>{m}</option>)}
              </select>
            </div>
            <div className="form-group">
          ...
          {hideBarrelCaliber && firearmType && <p className="form-hint">ℹ️ Barrel length and caliber are not required for {firearmType}.</p>}
          <hr className="section-divider" />
          ...
          <div className="form-row-3">
            ...
            {isCompany && <div className="form-group"><label className="form-label">Company Name <span className="required-mark">*</span></label><input type="text" className="form-control" value={mfgCompany} onChange={e => setMfgCompany(e.target.value)} /></div>}
          </div>
          <hr className="section-divider" />
          <div className="form-check" style={{ marginBottom: '0.75rem' }}>
            ...
            <div className="form-row-2">
              <div className="form-group">
                ...
                <select className="form-control" value={meansOfProduction} onChange={e => setMeansOfProduction(e.target.value)}>
                  <option value="">-- Select --</option>{MEANS_OF_PRODUCTION.map(m => <option key={m} value={m}>{m}</option>)}
                </select>
              </div>
              {meansOfProduction === 'Other' && <div className="form-group"><label className="form-label">Describe <span className="required-mark">*</span></label><input type="text" className="form-control" value={meansOther} onChange={e => setMeansOther(e.target.value)} /></div>}
            </div>
          )}
          <div className="btn-group">
        ...
        <div className="card">
          ...
          <div className="form-row-2">
            <div>
              ...
              <strong>Type:</strong> {requesterType === 'licensed' ? `Licensed — ${licenseType || licenseNo}` : 'Non-Licensed'}
            </div>
            <div>
              <strong>Firearm Type:</strong> {firearmType}<br />
          ...
          <div className="btn-group">
            ...
            <button className="btn btn-primary" onClick={() => { setSerialNumber(generateSerial()); setSubmitted(true); }}>Submit Request</button>
          </div>
        </div>
      )}
    </div>
