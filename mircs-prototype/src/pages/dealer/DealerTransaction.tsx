// REQ-0024, REQ-0025, REQ-0028, REQ-0031, REQ-0036, REQ-0039, REQ-0041, REQ-0184, REQ-0192
...
const MOCK_CUSTOMERS: Record<string, { name: string; licenseType: string; status: 'active' | 'expired' | 'suspended' | 'revoked'; expiry: string }> = {
  'LTC-12345': { name: 'John A. Smith', licenseType: 'License to Carry Firearms (LTC)', status: 'active', expiry: '2028-08-15' },
  'FID-88888': { name: 'Alice M. Brown', licenseType: 'Firearms Identification Card (FID)', status: 'active', expiry: '2029-01-20' },
  'FID-99999': { name: 'Mark T. Davis', licenseType: 'Firearms Identification Card (FID)', status: 'expired', expiry: '2024-03-01' },
  'LTC-SUSPEND': { name: 'Robert C. Jones', licenseType: 'License to Carry Firearms (LTC)', status: 'suspended', expiry: '2027-05-20' },
  'DLR-MA-00777': { name: 'Springfield Armory MA', licenseType: 'License to Sell/Rent/Lease Firearms and Ammunition', status: 'active', expiry: '2027-11-30' },
};
...
export default function DealerTransaction() {
  ...
  const [semiAutoFlags, setSemiAutoFlags] = useState<boolean[]>([false]);
  const [permitToPurchase, setPermitToPurchase] = useState(false);
  const [permitNo, setPermitNo] = useState('');
  const [submitted, setSubmitted] = useState(false);

  const isFID = customer?.licenseType?.includes('FID');
  const ticketNo = 'DLR-2026-' + Math.floor(Math.random() * 900000 + 100000);
  ...
  function lookupLicense() {
    ...
    setCustomer(c);
    if (c.status !== 'active') {
      setLicenseAlert(`TRANSACTION BLOCKED: This license is ${c.status.toUpperCase()}. Per M.G.L. c.140 §123(g), you cannot proceed with this sale.`);
    } else { setLicenseAlert(''); }
  }
    ...
    <div>
      ...
      {step === 1 && (
        <div className="card">
            ...
            <>
              <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'flex-end', marginBottom: '0.75rem' }}>
                <div className="form-group" style={{ flex: 1, margin: 0 }}>
                  <label className="form-label">{isDealerToDealer ? 'Dealer License No.' : 'Customer License No.'} <span className="required-mark">*</span></label>
                ...
                {!isDealerToDealer && <div className="form-group" style={{ margin: 0 }}><label className="form-label">PIN</label><input type="password" className="form-control" placeholder="PIN" style={{ width: 120 }} /></div>}
                <button className="btn btn-secondary" onClick={lookupLicense} type="button">Look Up</button>
              </div>
              {licenseAlert && <div className="alert alert-danger"><strong>⛔ {licenseAlert}</strong></div>}
              {customer && !licenseAlert && (
                <div className="alert alert-success">
                  <div className="license-valid valid" style={{ marginBottom: '0.5rem' }}>✅ License Valid {statusBadge(customer.status)}</div>
                  <strong>{customer.name}</strong><br />License Type: {customer.licenseType}<br />Expires: {customer.expiry}
                </div>
              )}
              {customer && !licenseAlert && (
                <div className="attest-box">
                  <div className="form-check">
                    ...
                    <label className="form-check-label" htmlFor="physical">
                      <strong>I confirm that I have verified the buyer's physical license documentation in person</strong> per M.G.L. c.140 §123(f) &amp; (k).
                    </label>
                  </div>
                </div>
              )}
          ...
          <div className="btn-group">
            ...
            <button className="btn btn-primary" disabled={!isOutOfStateTransfer && (!customer || !!licenseAlert || !physicalConfirmed)} onClick={() => setStep(2)}>Continue →</button>
          </div>
        </div>
      )}
      ...
      {step === 2 && (
        <div className="card">
          ...
          {firearms.map((fa, i) => (
            <div key={i}>
              <FirearmEntry firearm={fa} index={i} onChange={updateFirearm}
                onRemove={(idx) => { setFirearms(prev => prev.filter((_, j) => j !== idx)); setSemiAutoFlags(prev => prev.filter((_, j) => j !== idx)); }}
              ...
              {REQUIRES_SEMI_AUTO.includes(fa.type || '') && (
                <div style={{ marginTop: '-0.5rem', marginBottom: '1rem', padding: '0 1rem' }}>
                  <div className="form-check">
                    <input type="checkbox" id={`semi-${i}`} checked={semiAutoFlags[i] || false} onChange={e => setSemiAutoFlags(prev => prev.map((v, j) => j === i ? e.target.checked : v))} />
                  ...
                  {semiAutoFlags[i] && isFID && (
                    <div className="alert alert-danger" style={{ marginTop: '0.5rem' }}>
                      <strong>⛔ Sale Restricted:</strong> FID cardholders cannot purchase semi-automatic firearms per M.G.L. c.140 §129B.
                    </div>
                  )}
                </div>
              )}
          ...
          <button className="btn btn-secondary btn-sm" onClick={() => { setFirearms(prev => [...prev, {}]); setSemiAutoFlags(prev => [...prev, false]); }}>+ Add Another Firearm</button>
          <div className="btn-group">
        ...
        <div className="card">
          ...
          <div className="table-container">
            <table>
              ...
              <tbody>
                {firearms.map((f, i) => (
                  <tr key={i}><td>{i + 1}</td><td>{f.type}</td><td>{f.make}</td><td>{f.model}</td><td>{f.serialNumber}</td><td>{semiAutoFlags[i] ? '⚠️ Yes' : 'No'}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
